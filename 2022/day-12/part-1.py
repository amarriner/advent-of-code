...
file_handler = logging.FileHandler("day-12-part-1.log", mode="w")
...
if __name__ == '__main__':
    ...
    with open('input.txt') as f:
        ...
        for row, line in enumerate(f.read().split("\n")):
            ...
            for col, char in enumerate(line):
                ...
                distance = INFINITY
                if char == 'S':
                    grid.start = Point(row, col)
                    distance = 0
                    char = 'a'
                    grid.current = Point(row, col)
                elif char == 'E':
    ...
    grid = shortest_path(grid, queue)

    # -------------------------------------------------------------------------
    # RESULTS
    # -------------------------------------------------------------------------
    logging.info('-------------------------------------------------------------')
    logging.info(f'Start               : {grid.start}')
    ...
    logging.info(f'Shortest path to end: {grid.nodes[grid.end.row][grid.end.col].distance}')
    logging.debug(grid)
